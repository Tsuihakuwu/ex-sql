...
SELECT cli_ville, cli_nom, cli_prenom, cli_adresse FROM client WHERE cli_nom = 'White';
...
SELECT hot_nom, hot_categorie, hot_ville, sta_nom FROM hotel JOIN station ON sta_id = hot_sta_id;
...
SELECT hot_nom, hot_categorie, hot_ville, cha_numero FROM hotel JOIN chambre ON cha_hot_id = hot_id;
...
SELECT cli_nom, hot_nom, res_date_debut, datediff(res_date_fin,res_date_debut) AS duree_sejour FROM client
